...
- сравнивать фрагменты из нескольких баз;
- объединять найденный текст с результатами других инструментов;
- самостоятельно решать, достаточно ли контекста;
- выполнять повторный поиск;
- формировать ответ в специальном формате.

Минус этого подхода — в контекст агента передаются сырые фрагменты, поэтому они могут занимать больше токенов и
...
# При использовании RetrieverTool, свою функцию (search_trainbot_faq) писать не  обязательно
trainbot_rag_tool = RetrieverTool.from_defaults(
    retriever=retriever,
    name="trainbot_rag_tool",
    description=(
        "Ищет релевантные фрагменты в базе знаний TrainBot. "
...
async def run_agent(agent):
    ...
    """
    Здравствуйте! Остаток денежных средств на счёте можно вернуть, оформив заявку через форму обратной связи на нашем сайте trainbot.ru. 
    В заявке необходимо указать ваш ID пользователя — его можно найти в меню бота: Информация → Тарифы → ID.
    После проверки, остаток средств будет возвращён на вашу карту за вычетом комиссии эквайринга.
    Если возникнут сложности с оформлением — обязательно напишите, мы поможем! 🚆
    """
    return answer
